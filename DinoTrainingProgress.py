...
plt.rcParams["figure.figsize"] = (15, 9) #Makes the default argument of figure size (15, 9)
...
while interval <= len(scores_df):
    ...
    max_scores.loc[len(max_scores)] = (scores_df.loc[start:interval].mean()["Scores"])
    start = interval
    interval += 10
...
while interval <= len(q_max_df):
    q_max_scores.loc[len(q_max_scores)] = (q_max_df.loc[start:interval].mean()["qvalues"])
    start = interval
    interval += 1000
